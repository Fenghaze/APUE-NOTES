...
- 水平触发（电平一直为0或1就触发）：EPOLLLT（默认）
  ...
  - 比如远端发送10个字节的数据，服务端一次处理5个字节，TCP缓冲区还剩5个字节，数据需要读取两次才能将缓冲区读完，那么`epoll_wait()`就会一直触发两次
  - 优点：易于编码，未读完的数据下次还能继续读，不易遗漏
  - 缺点：在并发量高的时候，epoll_wait返回的就绪队列比较大，遍历比较耗时。因此LT适用于==并发量小==的情况
- ==边沿触发（电平由0变1或由1变0就触发）==：EPOLLET
  - **读缓冲区当有新的数据到达才会被触发；写缓冲区只要数据被发送了就会被触发**
  
  - 比如远端发送10个字节的数据，服务端一次处理5个字节，TCP缓冲区还剩5个字节，但是`epoll_wait()`只会触发一次，如果不使用循环读完，那么剩下的字节就无法读取；只有当**新的数据**发送过来时，才会再次触发，继续读剩下的字节
...
EPOLLIN：TCP读缓冲区**只要有数据就会一直触发**

EPOLLOUT：TCP写缓冲区**只要有空间就会一直触发**
...
ET 模式是一种边沿触发模型，在它检测到有 I/O 事件时，通过 epoll_wait 调用会得到有事件通知的文件描述符，对于每一个被通知的文件描述符，如可读，则必须将该文件描述符一直读到空，让 errno 返回 EAGAIN 为止，否则下次的 epoll_wait 不会返回余下的数据，会丢掉事件。而==如果你的文件描述符如果不是非阻塞的，那这个一直读或一直写势必会在最后一次阻塞==。
